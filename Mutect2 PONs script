...
# run the executable
# A script to generate PONs with gatk Mutect2, designed for the Phoenix supercomputer
# Variables that usually don't need changing once set for your system

usage()
{
echo "# A script to generate PONs with gatk Mutect2, designed for the Phoenix supercomputer
...
# 06/07/2018; Clare van Eyk; remove configuration and fix reference and dbSNP
...
if [ -z "$VcfFolder" ]; then # If no output directory then use default directory
        VcfFolder=$FASTDIR/WGS/Mosaic
        echo "Using $FASTDIR/WGS/Mosaic as the output directory"
fi
if [ ! -d $VcfFolder ]; then
        mkdir -p $VcfFolder
fi
...
## Start of the script ##
##To create panel of normals, run:##
###On each control sample###
cd $tmpDir
gatk Mutect2 \
...
-O $VcfFolder/${sampleID[$SLURM_ARRAY_TASK_ID]}.mosaic.vcf >> $tmpDir/${sampleID[$SLURM_ARRAY_TASK_ID]}.pipeline.log 2>&1
